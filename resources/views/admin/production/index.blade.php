...
<script>
(function(){
    const routeFind = "{{ route('admin.production.find-patient', ['predict3dId' => 'PREDICT_ID']) }}";
    const routeStepsGet = "{{ route('admin.production.steps.get', ['predict3dId' => 'PREDICT_ID']) }}";
    const routeStepsSave = "{{ route('admin.production.steps.save', ['predict3dId' => 'PREDICT_ID']) }}";
    ...
    function formatPhoneNumber(phone) {
        if (!phone) return '-';
        // Simple formatting - adjust based on your phone number format
        return phone.replace(/(\d{3})(\d{3})(\d{4})/, '$1-$2-$3');
    }

    // No save functionality needed here as cases are managed in patient details

    $btnFetch.addEventListener('click', async () => {
        const id = ($predict.value||'').trim();
        if(!id) { 
            alert('Please enter a Predict3D ID');
            return; 
        }
        ...
        try {
            ...
            // Update case displays and inputs
            ...
            // Also load existing production steps and populate grid
    ...
    $add.addEventListener('click', () => {
        ...
        const newIndex = headerRow.cells.length - 1; // before + button column
    ...
    // Helper: get number of data columns in the grid (excluding first label and plus column)
    function getDataColumnCount() {
        const headerCells = $table.tHead.rows[0].cells;
        return headerCells.length - 2; // minus Steps and plus button th
    }

    // Helper: ensure there are at least N columns
    function ensureColumns(n) {
        while (getDataColumnCount() < n) {
            $add.click();
        }
    }

    // Load steps and populate inputs
    async function loadProductionSteps(predictId) {
        try {
            ...
            steps.forEach(s => {
                const idx = parseInt(s.step_number); // 1-based
    ...
    $btnSaveSteps.addEventListener('click', async () => {
        if (!currentPredictId) {
            alert('Please fetch a patient first.');
            return;
        }
        ...
        for (let i = 1; i <= cols; i++) {
            ...
            if (hasAny) {
                steps.push({ step_number: i, upper: Number.isNaN(upperVal) ? null : upperVal, lower: Number.isNaN(lowerVal) ? null : lowerVal });
            }
        }
        try {
            ...
            console.error(e);
            alert(e && e.message ? e.message : 'Error saving production steps');
        }
    });
})();
</script>
