...
@section('title', 'Edit Doctor - SmileCare')
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.doctors.update', $doctor) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="chamber_name" name="chamber_name" value="{{ old('chamber_name', $doctor->chamber_name) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="chamber_address" class="form-label">Chamber address <span class="text-danger">*</span></label>
                            <textarea class="form-control" id="chamber_address" name="chamber_address" rows="3" required>{{ old('chamber_address', $doctor->chamber_address) }}</textarea>
                        </div>
                        <div class="mb-3">
                            <label for="chamber_map_link" class="form-label">Chamber Google Maps link <span class="text-muted">(optional)</span></label>
                            <input type="url" class="form-control" id="chamber_map_link" name="chamber_map_link" value="{{ old('chamber_map_link', $doctor->chamber_map_link) }}" placeholder="https://maps.google.com/...">
                            <small class="text-muted">Paste the full Google Maps location link for this chamber.</small>
                        </div>
                        <div class="mb-3">
                            <label for="mobile_number" class="form-label">Mobile number <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="mobile_number" name="mobile_number" value="{{ old('mobile_number', $doctor->mobile_number) }}" required placeholder="e.g. 01XXXXXXXXX">
                        </div>
                        ...
                        <input type="hidden" name="region_name" id="region_name" value="{{ old('region_name', $regionName) }}">
                        <input type="hidden" name="area_name" id="area_name" value="{{ old('area_name', $areaName) }}">
                        <input type="hidden" name="territory_name" id="territory_name" value="{{ old('territory_name', $territoryName) }}">

                        <div class="mb-3">
                            <label for="email" class="form-label">Email <span class="text-muted">(optional)</span></label>
                            <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $doctor->email) }}" placeholder="doctor@example.com">
                        </div>
                        <div class="mb-3">
                            <label for="note" class="form-label">Note</label>
                            <textarea class="form-control" id="note" name="note" rows="3" placeholder="Doctor description — e.g. specialization, practice focus, or other relevant details">{{ old('note', $doctor->note) }}</textarea>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="marketing_representative_name" name="marketing_representative_name" required>
                                ...
                                @foreach($mrOptions as $mr)
                                    <option value="{{ $mr }}" @selected(old('marketing_representative_name', $doctor->marketing_representative_name) === $mr)>{{ $mr }}</option>
                                @endforeach
                            </select>
                        </div>
...
(() => {
    ...
    const initialRegion = @json(old('region_name', $regionName));
    const initialArea = @json(old('area_name', $areaName));
    const initialTerritory = @json(old('territory_name', $territoryName));
    ...
    const form = document.querySelector('form[action="{{ route('admin.doctors.update', $doctor) }}"]');
    ...
    function refreshTerritoryOptions(forceAddNew = false) {
        if (forceAddNew) {
            ...
            territorySelect.disabled = false;
            if (territorySelect.value !== ADD_NEW) {
